...
src='D:\images'
dest_base='D:\glitch directories'

images_path=os.listdir(src)
...
for image in images_path:
    ...
    if c==1:
        ...
        f=os.path.join(dest_base,'1')
        cv2.imwrite(os.path.join(f,image), img)
        c=2
    elif c==2:
        img=glitch2(img)
        dim = (800, 600)
        img = cv2.resize(img, dim, interpolation = cv2.INTER_AREA)
        f=os.path.join(dest_base,'2')
        cv2.imwrite(os.path.join(f,image), img)
        c=3
    elif c==3:
        img=glitch3(img)
